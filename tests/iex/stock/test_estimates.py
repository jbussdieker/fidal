...
from fidal.iex.stock.estimates import fetch

iexvcr = vcr.VCR(path_transformer=vcr.VCR.ensure_suffix('.yaml'),
                 cassette_library_dir='tests/fixtures/cassettes/iex/stock/estimates',
                 decode_compressed_response=True,
                 filter_query_parameters=['token'])

class TestIEXStockEstimates(unittest.TestCase):
    @iexvcr.use_cassette()
    def test_fetch(self):
