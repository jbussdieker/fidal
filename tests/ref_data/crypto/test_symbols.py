...
from iex.ref_data.crypto.symbols import fetch

iexvcr = vcr.VCR(path_transformer=vcr.VCR.ensure_suffix('.yaml'),
                 cassette_library_dir='tests/fixtures/cassettes/ref_data/crypto',
                 decode_compressed_response=True,
                 filter_query_parameters=['token'])

class TestRefDataCryptoSymbols(unittest.TestCase):
    @iexvcr.use_cassette()
    def test_fetch(self):
